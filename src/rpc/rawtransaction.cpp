    ...
    return RPCHelpMan{"testmempoolaccept",
                ...
                RPCResult{
                    RPCResult::Type::ARR, "", "The result of the mempool acceptance test for each raw transaction in the input array.\n"
                        "Length is exactly one if any transaction fails validation.",
                    {
                        {RPCResult::Type::OBJ, "", "",
                        {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (all_valid) {
        for (unsigned int i = 0; i < validation_results.size(); ++i) {
            ...
            const CAmount fee = validation_results[i].m_base_fees;
        ...
        result_0.pushKV("txid", validation_results[0].ptx->GetHash().GetHex());
        result_0.pushKV("wtxid", validation_results[0].ptx->GetWitnessHash().GetHex());
