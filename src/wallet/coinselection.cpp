...
bool SelectCoinsSRD(std::vector<OutputGroup>& utxo_pool, const CAmount& target_value, std::set<CInputCoin>& out_set, CAmount& value_ret)
{
    ...
    for (const auto& group : utxo_pool) {
        selected_eff_value += group.effective_value;
...
void OutputGroup::Insert(const CInputCoin& output, int depth, bool from_me, size_t ancestors, size_t descendants, bool positive_only) {
    ...
    const CAmount coin_fee = output.m_input_bytes < 0 ? 0 : m_cs_params.m_effective_feerate.GetFee(output.m_input_bytes);
    ...
    coin.m_long_term_fee = coin.m_input_bytes < 0 ? 0 : m_cs_params.m_long_term_feerate.GetFee(coin.m_input_bytes);
...
{
    return m_cs_params.m_subtract_fee_outputs ? m_value : effective_value;
}
