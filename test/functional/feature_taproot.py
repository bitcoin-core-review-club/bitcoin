...
class TaprootTest(BitcoinTestFramework):
    ...
    def test_spenders(self, spenders, input_counts):
        ...
        self.log.info("Testing %i spenders in groups of %s" % (len(spenders), ", ".join("%i" % c for c in input_counts)))

        # Generate some coins to fund the wallet.
        self.nodes[0].generate(10)

        # Construct a bunch of sPKs that send coins back to the host wallet
        self.log.info("Constructing 16 addresses for returning coins")
        host_spks = []
        host_pubkeys = []
        for i in range(16):
        ...
        utxos = []
        while len(utxos) < len(spenders):
            # Compute how many UTXOs to create with this transaction
            count_this_tx = min(len(spenders) - len(utxos), (len(spenders) + 4) // 5, 10000)
            ...
            # Add outputs
            cur_progress = len(utxos) / len(spenders)
            next_progress = (len(utxos) + count_this_tx) / len(spenders)
            change_goal = (1.0 - 0.6 * next_progress) / (1.0 - 0.6 * cur_progress) * balance
            self.log.info("Create %i UTXOs in a transaction spending %i inputs worth %.8f (sending ~%.8f to change)" % (count_this_tx, len(unspents), balance * 0.00000001, change_goal * 0.00000001))
            for i in range(count_this_tx):
                avg = (balance - change_goal) / (count_this_tx - i)
                amount = int(random.randrange(int(avg*0.85 + 0.5), int(avg*1.15 + 0.5)) + 0.5)
                balance -= amount
                fund_tx.vout.append(CTxOut(amount, spenders[len(utxos) + i].script))
            # Add change
            fund_tx.vout.append(CTxOut(balance - 10000, random.choice(host_spks)))
            # Ask the wallet to sign
            ss = BytesIO(bytes.fromhex(self.nodes[0].signrawtransactionwithwallet(ToHex(fund_tx))["hex"]))
            fund_tx.deserialize(ss)
            # Construct UTXOData entries
            fund_tx.rehash()
            for i in range(count_this_tx):
                utxos.append(UTXOData(outpoint=COutPoint(fund_tx.sha256, i), output=fund_tx.vout[i], spender=spenders[len(utxos)]))
            # Mine into a block
            self.block_submit(self.nodes[0], [fund_tx], "Funding tx", None, random.choice(host_pubkeys), 10000, MAX_BLOCK_SIGOPS_WEIGHT, True, True)
        ...
        while len(utxos):
            ...
            acceptable = [n for n in input_counts if n <= len(utxos) and (len(utxos) - n > max(input_counts) or (len(utxos) - n) in [0] + input_counts)]
            num_inputs = random.choice(acceptable)

            # Chop off that many UTXOs from utxos.
            input_utxos = []
            for _ in range(num_inputs):
                input_utxos.append(utxos.pop())
            ...
            tx.wit.vtxinwit = [CTxInWitness() for _ in range(num_inputs)]
            ...
            # Add 1 to 4 random outputs.
            num_outputs = random.choice([1, 2, 3, 4])
            ...
            for fail_input in list(range(num_inputs)) + [None]:
                ...
                if is_standard_tx:
                    ...
                    assert self.nodes[0].getmempoolentry(tx.hash) is not None, "Failed to accept into mempool: " + msg
                else:
                    assert_raises_rpc_error(-26, None, self.nodes[0].sendrawtransaction, tx.serialize().hex(), 0)
                # Submit in a block
                self.block_submit(self.nodes[0], [tx], msg, witness=True, accept=fail_input is None, cb_pubkey=cb_pubkey, fees=fee, sigops_weight=sigops_weight, err_msg=expected_fail_msg)
    ...
    def run_test(self):
        ...
        # Run every test, in transactions that spend 1-3 inputs at once.
        spenders = self.build_spenders()
        self.test_spenders(spenders, input_counts=[1, 2, 2, 2, 3])
