...
def spenders_taproot_active():
    ...
    scripts = [
        ...
        ("t28", CScript([checksigadd_arg, pubs[1], OP_CHECKSIGADD, checksigadd_result, OP_EQUAL])),
        ...
        ("t32", CScript([checksigadd_arg, b'\x03' + pubs[1], OP_CHECKSIGADD, checksigadd_result, OP_EQUAL])),
...
class TaprootTest(BitcoinTestFramework):
    ...
    def test_spenders(self, node, spenders, input_counts):
        ...
        self.log.info("- Running %i spending tests" % len(utxos))
        ...
        while len(utxos):
            ...
            if (len(spenders) - len(utxos)) // 200 > (len(spenders) - len(utxos) - len(input_utxos)) // 200:
                self.log.info("  - %i tests done" % (len(spenders) - len(utxos)))
